...
class BaseModel(object):
    ...
    def get_package_list(self):
        ...
        val = [pp.name[0].upper() for pp in self.packagelist]
        return val
    ...
    def run_model(self, silent=False, pause=False, report=False,
        ...
        return run_model(self.exe_name, self.namefile, model_ws=self.model_ws,
                         ...
                         normal_msg=normal_msg, async=async)
    ...
    def name(self):
        ...
        """
        return self.__name  # copy.deepcopy(self.__name)
...
def run_model(basestring exe_name, basestring namefile, basestring model_ws='./',
              bint silent=False, bint pause=False, bint report=False,
              object normal_msg='normal termination',
              bint async=False, object cargs=None):
    """
    ...
    cdef list argv
    cdef Py_ssize_t idx
    cdef basestring s, c, exe, line, target_msg  # msg
    cdef object proc
    ...
    if not async:
        # proc.wait()
        # return True, []  # just assume success...

        target_msg = normal_msg[0]
        # for line in iter(proc.stdout.readline, ''):
        for c in iter(proc.stdout.readline, ''):
            # c = line.decode('utf-8')
            # c = c.rstrip('\r\n')
