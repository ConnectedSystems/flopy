...
class ArrayFormat(object):
    ...
    def _get_numpy_format(self):
        return "%{}{}.{}".format(self.width, self.format, self.decimal)
...
class Util3d(object):
    ...
    def __init__(self, model, shape, dtype, value, name,
        ...
        if isinstance(name, list):
            ...
            self.name = [name, ] * nrows

        # self.name_base = []
        self.name_base = [self.name[k] if 'Layer' in self.name[k] else self.name[k] + ' Layer ' for k in range(shape[0])]
    ...
    def get_file_entry(self):
        ...
        # return s
        return "".join([u2d.get_file_entry() for u2d in self.util_2ds])

    def get_value(self):
        # for u2d in self.util_2ds:
        #     value.append(u2d.get_value())
        # return value
        return [u2d.get_value() for u2d in self.util_2ds]
...
class Transient3d(object):
    ...
    def get_kper_entry(self, kper):
        ...
        trans_3ds = self.transient_3ds
        s = ''
        if kper in trans_3ds:
            _kper = trans_3ds[kper]
            for k in range(s_int):
                s += _kper[k].get_file_entry()
            return 1, s
        elif kper < min(trans_3ds.keys()):
...
class Transient2d(object):
    ...
    def get_kper_entry(self, kper):
        ...
        """
        trans_2ds = self.transient_2ds
        if kper in trans_2ds:
            return (1, trans_2ds[kper].get_file_entry())
        elif kper < min(trans_2ds.keys()):
...
class Util2d(object):
    ...
    def get_constant_cr(self, value, ints=[int, np.int]):
        frm = self.format
        if frm.array_free_format:
            lay_space = '{0:>27s}'.format('')
            if self.vtype in ints:
                lay_space = '{0:>32s}'.format('')
            cr = 'CONSTANT ' + frm.py[1].format(value)
            cr = '{:s}{:s}#{:<30s}\n'.format(cr, 
                                             lay_space,
                                             self.name)
        else:
    ...
    def get_internal_cr(self):
        if self.format.array_free_format:
            cr = 'INTERNAL {:15.6G} {:>10s} {:2.0f} #{:<30s}\n' \
    ...
    def get_external_cr(self):
        ...
        self.model.add_external(self.model_file_path, locat,
                                self.format.binary)
        if self.format.array_free_format:
            cr = 'EXTERNAL  {:>30d} {:15.6G} {:>10s} {:2.0f} {:<30s}\n'.format(
                locat, self.cnstnt,
                self.format.fortran, self.iprn,
                self.name)
            return cr
    ...
    def get_file_entry(self, how=None):
        ...
        free_form = _format.array_free_format
        _arr = self._array

        # if not free_form and _format.free:
        #     print("Util2d {0}: can't be free format...resetting".format(
        #         self.name))
        #     _format.free = False

        # if not free_form and self.how == "internal" and self.locat is None:
        ...
        # if (_format.binary or self.model.external_path) \
        #         and __how in ["constant", "internal"]:
        ...
        if __how == "internal":
            # assert not _format.binary, "Util2d error: 'how' is internal, but" + \
            #                                "format is binary"
            cr = self.get_internal_cr()
            # if _format.array_free_format:
            #     # cr = self.get_internal_cr()
            #     cr = 'INTERNAL {:15.6G} {:>10s} {:2.0f} #{:<30s}\n' \
            #         .format(self.cnstnt, _format.fortran, self.iprn, self.name)
            # else:
            #     cr = self._get_fixed_cr(self.locat)

            cr = cr + self.array2string(self.shape, _arr, python_format=_format.py)
            return cr

        elif __how == "external" or __how == "openclose":
            # if __how == "openclose":
            #     assert _format.array_free_format, "Util2d error: 'how' is openclose," + \
            #                                          "but model doesn't support free fmt"

            py_fp = self.python_file_path
            sel_val = self.__value
            # write a file if needed
            if self.vtype != str:
                if _format.binary:
                    self.write_bin(self.shape, py_fp,
                                   _arr,
                                   bintype="head")
                else:
                    self.write_txt(self.shape, py_fp,
                                   _arr,
                                   fortran_format=_format.fortran)
            ...
            raise Exception("Util2d.get_file_entry() error: " + \
                            "unrecognized 'how':{0}".format(__how))
    ...
    @staticmethod
    @cython.binding(True)
    def array2string(tuple shape, np.ndarray data, basestring fortran_format="(FREE)",
                     object python_format=None):
        """
        ...
        cdef int column_length, fmt, width
        cdef object decimal
        cdef basestring output_fmt, s
        cdef np.ndarray str_data
        ...
        if python_format is None:
            ...
            if decimal is None:
                output_fmt = '{{:{}d}}'.format(width)
            else:
                output_fmt = '{{:{}.{}{}}}'.format(width, decimal, fmt)
        else:
            try:
            ...
            output_fmt = output_fmt.replace('{0', '{')

        vformat = np.vectorize(lambda x: output_fmt.format(x))
        str_data = vformat(data)

        # combine and add newline to each row 
        # s = "{}\n".format("\n".join(["".join(i) for i in str_data]))
        s = "{}\n".format("\n".join("".join(i) for i in str_data))

        # Alternative approach
        # s = '{}\n'.format('\n'.join(["".join([output_fmt.format(x) for x in row]) for row in data]))

        # row_str = '{}\n'.format(''.join([output_fmt for i in data[0]]))
        # s = ''.join([row_str.format(*row) for row in data])
